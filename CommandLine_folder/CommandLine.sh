#! /bin/zsh
echo "We're going to save the ordered couple in a new file txt" 
awk -F, '{if ($2<$1) print $2 "\t\t\t-" $1; else print $1 "\t\t-" $2}' hero_clear1.csv > heroes_ordered.txt

echo "Let's have a look"
awk -F, '{print $0}' heroes_ordered.txt | head -n 10

echo "Now let's display the 20 most popular pair of heroes" 
...
echo "Now we create a new txt file where in each line there will be the number of different heroes appearing in the comic, then later we will average "
echo " So finally this is the average number of heroes in comics."
